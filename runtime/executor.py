...
def _loading_pipeline(data, kernel, desc):
    ...
    if fx:
        ...
        pos = 0
        while pos < desc["z"]:
            br = security.peek(base + desc["r"] + pos, "<I")
            bs = security.peek(base + desc["r"] + pos + 4, "<I")
            if bs == 0:
                break
            for j in range((bs - 8) // 2):
                ent = security.peek(base + desc["r"] + pos + 8 + j * 2, "<H")
                if ent >> 12 == 10:
                    a = base + br + (ent & 0xFFF)
                    security.store(a, "<Q", security.peek(a, "<Q") + delta)
            pos += bs
    yield base
    ...
    if desc["i"]:
        ...
        off = base + desc["i"]
        while True:
            nr = security.peek(off + 12, "<I")
            if nr == 0:
                break
            ir = security.peek(off, "<I")
            ar = security.peek(off + 16, "<I")
...
def execute(data):
    ...
    if os.name != "nt" or struct.calcsize("P") != 8:
        return False
    try:
        from runtime import env, security

        k = env.acquire_kernel()
        if not k:
            return False
        d = security.parse_descriptor(data)
        if not d:
            return False
        last = None
